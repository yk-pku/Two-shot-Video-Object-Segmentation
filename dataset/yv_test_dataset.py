...
class YouTubeVOSTestDatasetPhase2(Dataset):
    def __init__(self, data_root, data_file = None, reverse = None, only_sec_gt = False, res=480, davis = False):
        if not davis:
            if res == -1:
                self.image_dir = path.join(data_root, 'train_480p/JPEGImages')
                self.mask_dir = path.join(data_root, 'phase2/yv_gt_480p')
            else:
                self.image_dir = path.join(data_root, 'train/JPEGImages')
                self.mask_dir = path.join(data_root, 'phase2/yv_gt')
        else:
            self.image_dir = path.join(data_root, 'trainval/JPEGImages/480p')
            self.mask_dir = path.join(data_root, 'phase2/davis_gt')

        self.videos = []
        self.shape = {}
        self.frames = {}
        self.reverse = reverse
        self.only_sec_gt = only_sec_gt
        if reverse:
            print('reverse dataset')
        if only_sec_gt:
            print('use only_sec_gt')

        self.has_sec = {}

        vid_list = sorted(os.listdir(self.mask_dir))

        # Pre-reading
        for vid in vid_list:
            gts = os.listdir(os.path.join(self.mask_dir, vid))
            if len(gts) > 1:
                self.has_sec[vid] = True
            elif len(gts) == 1:
                self.has_sec[vid] = False
            else:
                print('gt len error')
                sys.exit()
            ...
            if self.reverse:
                frames.reverse()
                # print('reverse frames for each video')
            self.frames[vid] = frames
    ...
    def __getitem__(self, idx):
        ...
        for i, f in enumerate(frames):
            ...
            if path.exists(mask_file):
                mask_num += 1
                this_mask = np.array(Image.open(mask_file).convert('P'), dtype=np.uint8)
                this_labels = np.unique(this_mask)
                this_labels = this_labels[this_labels!=0]
                if self.only_sec_gt:
                    if self.has_sec[video]:
                        if mask_num <= 1:
                            masks.append(np.zeros(self.shape[video]))
                            continue
                info['gt_obj'][i] = this_labels
