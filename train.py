...
if local_rank == 0:
    ...
    model = STCNModel(para, logger=logger,
                    save_path=save_path if long_id is not None else None,
...
def renew_vos_loader(max_skip):
    ...
    yv_dataset = VOSDataset(yv_data_root, yv_gt_root,
                        max_skip//5, is_bl=False, data_file = para['yv_data'], pseudo_gt_root = yv_p_gt_root)
    davis_dataset = VOSDataset(davis_data_root, davis_gt_root,
                        max_skip, is_bl=False, data_file = para['davis_data'], pseudo_gt_root = davis_p_gt_root)
...
if para['stage'] == 0:
    ...
    yv_data_root = path.join(path.expanduser(para['yv_root']), 'train_480p', 'JPEGImages')
    davis_data_root = path.join(path.expanduser(para['davis_root']), '2017', 'trainval', 'JPEGImages', '480p')

    yv_gt_root = path.join(path.expanduser(para['yv_root']), 'train_480p', 'Annotations')
    davis_gt_root = path.join(path.expanduser(para['davis_root']), '2017', 'trainval', 'Annotations', '480p')

    if para['phase2_train'] >= 0:
        print('Phase2_train activated!')
        yv_p_gt_root = path.join(path.expanduser(para['phase2_root']), para['phase2_yv'], 'Annotations')
        davis_p_gt_root = path.join(path.expanduser(para['phase2_root']), para['phase2_davis'], 'Annotations')
    else:
        yv_p_gt_root = None
        davis_p_gt_root = None
...
try:
    for e in range(current_epoch, total_epoch):
        ...
        for data, semi_data in train_loader:
            model.do_pass(data, semi_data, total_iter, e)
            total_iter += 1
